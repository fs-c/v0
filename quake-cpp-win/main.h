...
//vsgetconstantbuffers
ID3D11Buffer *vssBuffer;
UINT vssNumBuffers;
D3D11_BUFFER_DESC vssdesc;
...
{
	ofstream fout;
	fout.open(GetDirectoryFile("qcd3d.ini"), ios::trunc);
...
void LoadCfg()
{
	ifstream fin;
	string Word = "";
	fin.open(GetDirectoryFile("qcd3d.ini"), ifstream::in);
	fin >> Word >> Item1;
	fin >> Word >> Item2;
	fin >> Word >> Item3;
	fin >> Word >> Item4;
...
//w2s
int ProjCBnum = 4;
int matProjnum = 0;//0 = 80% players and 32 = 20% players

ID3D11Buffer* pProjCB = nullptr;
ID3D11Buffer* m_pCurProjCB = NULL;

void AddModelA(ID3D11DeviceContext* pContext)
{
	pContext->VSGetConstantBuffers(4, 1, &pProjCB);//ProjCBnum changed to 4
	...
	DirectX::XMMATRIX WorldViewProj = DirectX::XMMatrixTranspose((FXMMATRIX)*matProj); //transpose
...
void AddModelB(ID3D11DeviceContext* pContext)
{
	pContext->VSGetConstantBuffers(4, 1, &pProjCB);//ProjCBnum changed to 4
	...
	float matProj[4][4];
	{
		float *proj;
		MapBuffer(m_pCurProjCB, (void**)&proj, NULL);
		memcpy(matProj, &proj[32], sizeof(matProj));//matProjnum changed to 32
		UnmapBuffer(m_pCurProjCB);
	}
	//SAFE_RELEASE(pWorldViewCB);
	//SAFE_RELEASE(pProjCB);
