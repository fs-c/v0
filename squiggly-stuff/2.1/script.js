...
const points = Array(segments).fill().map(() => {
    const rad = rb(0, 2 * Math.PI);
...

{
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ctx.bezierCurveTo(
            horizontalRoot + points[i][0] + rv(horizontalDeviation), verticalRoot + points[i + 1][0] + rv(verticalDeviation),
            horizontalRoot + points[i][0] + rv(horizontalDeviation), verticalRoot + points[i + 1][1] + rv(verticalDeviation),
